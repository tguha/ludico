...
static const auto spritesheet =
    SpritesheetResource(
        "item_seed_sunflower",
        "item/seed_sunflower.png",
        { 8, 8 });

TextureArea ItemSeedSunflower::sprite() const {
    return spritesheet[uvec2(0, 0)];
}

EntitySpawnFn ItemSeedSunflower::spawn_fn(
    const ItemStack &item,
    const Entity &entity,
    const ivec3 &pos,
    Direction::Enum face) const {
    return [](Level &level, const vec3 &pos) {
        return level.spawn<EntitySunflower>(pos);
    };
}

InteractionAffordance ItemSeedSunflower::can_plant(
    const ItemStack &item,
    const Entity &entity,
    const ivec3 &pos,
    Direction::Enum face) const {
...
DECL_ITEM_REGISTER(ItemSeedSunflower)
