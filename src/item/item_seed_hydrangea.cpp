...
static const auto spritesheet =
    SpritesheetResource(
        "item_seed_hydrangea",
        "item/seed_hydrangea.png",
        { 8, 8 });

TextureArea ItemSeedHydrangea::sprite() const {
    return spritesheet[uvec2(0, 0)];
}

EntitySpawnFn ItemSeedHydrangea::spawn_fn(
    const ItemStack &item,
    const Entity &entity,
    const ivec3 &pos,
    Direction::Enum face) const {
    return [](Level &level, const vec3 &pos) {
        return level.spawn<EntityHydrangea>(pos);
    };
}

InteractionAffordance ItemSeedHydrangea::can_plant(
    const ItemStack &item,
    const Entity &entity,
    const ivec3 &pos,
    Direction::Enum face) const {
...
DECL_ITEM_REGISTER(ItemSeedHydrangea)
