...
/*
   ...
   MODIFICATIONS BY JDH (jdh@jdh.gg)
*/
...
namespace result {
    ...
    namespace details {
        ...
        namespace impl {
            ...
            struct result_of<Ret (Cls::*)(Args...)> : public result_of<Ret(
                Args...)> {
            };
...
// Specialization for a void callback returning a Result
            ...
            template<typename Ret, typename Cls, typename... Args>
            struct Map<Ret (Cls::*)(Args...) const> : public impl::Map<Ret(
                Args...)> {
            };

            template<typename Ret, typename... Args>
            struct Map<std::function<Ret(Args...)>> : public impl::Map<Ret(
                Args...)> {
            };
        ...
        namespace err {
            ...
            namespace impl {
                ...
                struct Map<Ret (Cls::*)(Arg) const> {
                    ...
                    static Result<void, Ret>
                    map(const Result<void, E> &result, Func func) {
                        if (result.isErr()) {
                            auto res = func(result.storage().template get<E>());
                            return types::Err<Ret>(res);
                        }
        ...
        namespace And {
            ...
            namespace impl {
                ...
                template<typename Ret, typename Cls, typename... Args>
                struct Then<Ret (Cls::*)(Args...) const> : public Then<Ret(
                    Args...)> {
                };
            ...
            template<typename Ret, typename Cls, typename... Args>
            struct Then<Ret (Cls::*)(Args...)> : public impl::Then<Ret(
                Args...)> {
            };

            template<typename Ret, typename Cls, typename... Args>
            struct Then<Ret (Cls::*)(Args...) const> : public impl::Then<Ret(
                Args...)> {
            };
        ...
        namespace Or {
            ...
            namespace impl {
                ...
                template<typename Ret, typename Cls, typename... Args>
                struct Else<Ret (Cls::*)(Args...) const> : public Else<Ret(
                    Args...)> {
                };
                ...
                struct Else<Result<T, F>(Arg)> {
                    ...
                    template<typename E, typename Func>
                    static Result<void, F>
                    orElse(const Result<void, E> &result, Func func) {
                        if (result.isErr()) {
                            auto res = func(result.storage().template get<E>());
                            return res;
                        }
            ...
            template<typename Ret, typename Cls, typename... Args>
            struct Else<Ret (Cls::*)(Args...)> : public impl::Else<Ret(
                Args...)> {
            };

            template<typename Ret, typename Cls, typename... Args>
            struct Else<Ret (Cls::*)(Args...) const> : public impl::Else<Ret(
                Args...)> {
            };
        ...
        namespace Other {
            ...
            namespace impl {
                ...
                template<typename Ret, typename Cls, typename... Args>
                struct Wise<Ret (Cls::*)(Args...) const> : public Wise<Ret(
                    Args...)> {
                };
            ...
            template<typename Ret, typename Cls, typename... Args>
            struct Wise<Ret (Cls::*)(Args...)> : public impl::Wise<Ret(
                Args...)> {
            };

            template<typename Ret, typename Cls, typename... Args>
            struct Wise<Ret (Cls::*)(Args...) const> : public impl::Wise<Ret(
                Args...)> {
            };
        ...
        struct Storage {
            ...
            static constexpr size_t Align =
                sizeof(T) > sizeof(E) ? alignof(T) : alignof(E);
        ...
        struct Constructor {
            ...
            static void
            copy(const Storage<T, E> &src, Storage<T, E> &dst, ok_tag) {
            ...
            static void
            copy(const Storage<T, E> &src, Storage<T, E> &dst, err_tag) {
        ...
        struct Constructor<void, E> {
            static void
            move(Storage<void, E> &&src, Storage<void, E> &dst, ok_tag) {
            }

            static void
            copy(const Storage<void, E> &src, Storage<void, E> &dst, ok_tag) {
            }

            static void
            move(Storage<void, E> &&src, Storage<void, E> &dst, err_tag) {
                dst.rawConstruct(std::move(src.template get<E>()));
                src.destroy(err_tag());
            }

            static void
            copy(const Storage<void, E> &src, Storage<void, E> &dst, err_tag) {
                dst.rawConstruct(src.template get<E>());
            }
        };
    ...
    struct Result {
        ...
        static_assert(
            !std::is_same<E, void>::value, "void error type is not allowed");
        ...
        unwrap(
            ...
            const usize _line = __builtin_LINE()) const {
            if (isOk()) {
                return storage().template get<U>();
            }
            ...
            log::out(
                ...
                _line,
                _caller,
                "Attempting to unwrap an error Result");
            std::terminate();
        }
        ...
        E unwrapErr(
            ...
            log::out(
                ...
                _line,
                _caller,
                "Attempting to unwrapErr an OK Result");
            std::terminate();
        }
    ...
    private:
        ...
        T expect_impl(
            std::false_type) const { return storage_.template get<T>(); }
    ...
    bool operator==(const Result<T, E> &lhs, const Result<T, E> &rhs) {
        static_assert(
            concept_::EqualityComparable<T>::value,
            "T must be EqualityComparable for Result to be comparable");
        static_assert(
            concept_::EqualityComparable<E>::value,
...
template <typename E>
inline auto make_err(E &&e) {
    return result::Err(std::forward<E>(e));
}

template <typename T>
inline auto make_ok(T &&t) {
    return result::Ok(std::forward<T>(t));
}
